...
class EvaluationDataloader(Dataset):
    def __init__(self, data_dir, max_depth=None, sample_size=None):
        """
        Initialize the dataloader with directory path and optional max depth clipping.
        All .tiff files are loaded as depth images and all .png files as RGB images.
        """
        self.data_dir = data_dir
        self.max_depth = max_depth

        # Load all .tiff and .png files from the directory
        ...
        # If a sample size is provided, randomly sample the files
        if sample_size:
        ...
        # Sort the file lists to maintain consistent ordering
    ...
    def __len__(self):
        # Return the minimum length to avoid index errors if files count mismatch
        return min(len(self.depth_files), len(self.rgb_files))

    def __getitem__(self, idx):
        # Construct full file paths for the depth and RGB images
        depth_path = os.path.join(self.data_dir, self.depth_files[idx])
        rgb_path = os.path.join(self.data_dir, self.rgb_files[idx])

        # Read depth image in unchanged mode and RGB image in color mode
        depth = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED).astype(np.float32)
        rgb = cv2.imread(rgb_path, cv2.IMREAD_COLOR)
        if depth is None or rgb is None:
            raise ValueError(f"Error loading images at index {idx}: {depth_path} or {rgb_path}")

        # Optionally clip depth values above max_depth to zero
        if self.max_depth is not None:
            logging.warning(f"Max Depth Mode Enabled: Clipping depth values greater than {self.max_depth} to 0")
            depth[depth > self.max_depth] = 0

        # Convert BGR (OpenCV default) to RGB
        rgb = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)

        # Use the new camera intrinsics provided
        intrinsics = np.array([[2736.0, 0.0, 2736.0],
                               [0.0, 2736.0, 1824.0],
                               [0.0, 0.0, 1.0]], dtype=np.float32)

        return {
            'depth': depth,                     # Depth image array
            'rgb': rgb,                         # RGB image array
            'depth_filename': self.depth_files[idx],
...
if __name__ == "__main__":
    ...
    #checkpoint_path = "/home/rog-nuc/Downloads/ViTb-Final-Training/custom-trainingv4-ViTb_vitb_5e-06_0.01_epochs56_epoch20.pth"

    # Path to Evaluation Data
    data_dir = f"/home/rog-nuc/Desktop/ashwin-thesis-workspace/mde-data/livox-flir-pipeline-benchmark/Navvis-Evaluation/navvs-augmented-dense-depth"
    ...
    # Model Parameters
    model = 'DepthAnythingv2-vKiTTi-Navvis-ViTs'
    model_configs = {
    ...
    for batch in navvis_dataloader:
        filename = batch['depth_filename']
        #text_filename = filename.pop().replace(".tiff", "")
        ...
        for filename, rgb, depth in zip(filename, rgb_batch, depth_batch):  # Iterate over individual RGB and depth pairs
            ...
            predicted_pcd = generate_pointcloud(rgb, predicted, intrinsics, f"{output_metrics_path}{filename}_predicted.ply", save=SAVE)
            ground_truth_pcd = generate_pointcloud(rgb, depth, intrinsics, f"{output_metrics_path}{filename}_ground_truth.ply", save=SAVE)
            ...
            pointcloud_metrics = compute_pointcloud_metrics(
                np.asarray(predicted_pcd.points), 
                np.asarray(ground_truth_pcd.points)
)
